...
hotEncTrainAll <- fread('../withoutCentre/2012_2016/expandES_II_Month_preProcessed_training_2012_2016_noCentre.csv', data.table=FALSE)



hotEncTrainAll$V1 <- NULL

hotEncTrainAll$OpDate <- NULL
hotEncTrainAll$yearMonth <- NULL
hotEncTrainAll$Year <- NULL
hotEncTrainAll$ES_II_link_ID <- NULL
colnames(hotEncTrainAll)


#hotEncTrainTop18_graph <- sparsebnData(hotEncTrainTop18, type = "discrete")
hotEncTrainTopAll_graph <- sparsebnData(hotEncTrainAll, type = "continuous")


#Structure learning
hotEncTrainTopAll_graph.learn <- estimate.dag(hotEncTrainTopAll_graph)

#Model selection

selected.lambda <- select.parameter(hotEncTrainTopAll_graph.learn, hotEncTrainTopAll_graph) #5
...
plot(hotEncTrainTopAll_graph.learn[[selected.lambda]], vertex.label = get.nodes(hotEncTrainTopAll_graph.learn[[selected.lambda]])
     , vertex.size = 10
